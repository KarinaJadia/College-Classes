...
static void RunFIFO(int *listPageRequests, int numPageRequests, int memoryCapacity, int numPageFaultsExpected)
{
    ...
    // Build page history by accessing all pages
    for(int i=0; i<numPageRequests; ++i)
    {
      // access the page
      mem.AccessPage(listPageRequests[i]);
    ...
    // now get FIFO result
    int numPageFaults = mem.RunFIFO();

    // FIFO evicts oldest page (first-in)
    ASSERT_EQ( numPageFaults, numPageFaultsExpected);
}
...
static void Test()
{
    ...
    // find the FIFO missing
    cout << "Now run the FIFO algorithm...\n";
    RunFIFO(pageList, 12, 4, 9);
}
