...
public:
    Rectangle(T width, T height)
      : w(width >= 0 ? width : -width), h(height >= 0 ? height : -height) {}

    Rectangle(T x1, T y1, T x2, T y2)
      : w(std::abs(x2 - x1)), h(std::abs(y2 - y1)) {}

    T CalcArea() const override { return w * h; }
...
public:
    explicit Square(T side) : Rectangle<T>(side, side) {}
};