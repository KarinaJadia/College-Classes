...
void * printer_main(void * arg)
{
    ...
    while (1) {
        ...
        if (pthread_mutex_lock(&(jq->mutex)) != 0) die("pthread_mutex_lock");

        if (q_num_jobs(jq) > 0) jt = q_fetch_job(jq, pprinter->id);  
        else {
            pthread_mutex_unlock(&(jq->mutex));
            break;
        }

        if (pthread_mutex_unlock(&(jq->mutex)) != 0) die("pthread_mutex_unlock");
...
int main(int argc, char *argv[])
{
     ...
     *  */
    // prepare the argument to threads and create printer threads

    pthread_t printer_threads[num_printers];

    if (pthread_mutex_init(&job_queue.mutex, NULL) != 0) die("pthread_mutex_init");
    for(int i = 0; i < num_printers; i++) {
        printers[i].id = i;
        printers[i].jq = &job_queue;
        if (pthread_create(&printers[i].thread_id, NULL, printer_main, &printers[i]) != 0) die("pthread_create");
    }
    for (int i = 0; i < num_printers; i++) {
        if (pthread_join(printers[i].thread_id, NULL) != 0) die("pthread_join");
    }
