...
// generates samples for each cluster
...
// computes labels for data points based on centroids
...
//computes centroids based on labels
...
//computes squared intra-cluster distance
...
//runs k-means clustering and print results
...
void* thread_main(void* thread_arg) {
    ...
    // initialize centroids for this thread
    for(int j = 0; j < k; j++) {
        int idx = rand_r(&id) % m;
