...
void    child_main(int fdp[], int fdc[], int seed) {
    ...
    close(fdp[1]); // parent write close
    close(fdc[0]); // child read close

    //  send max value to the parent 
    int maxVal = gmn_get_max();
    if ( write(fdc[1], &maxVal, sizeof(int)) < 0 ) die("error");
    ...
    //      wait for a guess from parent 
        if (read(fdp[0], &guess , sizeof(int)) < 0) {
            die("failed");
        }
    ...
    //      send the result to parent
        ...
        if (write(fdc[1], &check, sizeof(int)) < 0) {
            die("failed");
        }
    }
    ...
    if (write(fdc[1], msg, MSG_BUF_SIZE) < 0) {
        die("error");
    }
...
int main(int argc, char *argv[]) {
    ...
    if (read(fdc[0], &max, sizeof(int)) < 0) {
        die ("error");
    }
    ...
    // write(fdp[0], stdout, strlen(stdout));
	// printf("result = %d\n", result);

    char msg[MSG_BUF_SIZE];
    if(read(fdc[0],msg, MSG_BUF_SIZE) < 0) {
            die("error");
        }
