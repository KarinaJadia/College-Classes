...
void start_program(Program *programs, int num_programs, int cur)  {
    // TODO
    // start the program  This function does not wait for the child process to finish.

    pid_t child;
    child = fork();
   
    if(child < 0) perror("fork"), die("error entering child");

    if (child == 0) { // enter child function

        if (cur > 0) dup2(programs[cur].fd_in, FD_STDIN), close(programs[cur].fd_in); // redirects standin
        
        if (cur != (num_programs-1)) dup2(programs[cur].fd_out, FD_STDOUT), close(programs[cur].fd_out); // redirects standout
       
        if (cur != num_programs-1) { // if the current program is not the last one in the pipeline
            for (int i = cur + 1; i < (num_programs - 1); i++) { // loop through programs
                
                // close input and output pipe of current program
                close(programs[i].fd_in);
                close(programs[i].fd_out);

            }
            close(programs[num_programs-1].fd_in); // close input pipe
        }
    
        execvp(programs[cur].argv[0], programs[cur].argv); // if successful, error code below won't execute
        die("error");
        exit(-1);
    }
    ...
    // closing pipes
    if (cur!=0) close(programs[cur].fd_in);
    if (cur != num_programs - 1) close(programs[cur].fd_out);
}
...
{
    // TODO
    // create the numbers needed for the program

    int readwrite[2]; // read and write functions

    programs[0].fd_in = 0; // intializes input pipe

    for (int i = 0; i < num_programs - 1; i++) { // loop through programs to create pies for them
        if(pipe(readwrite) == -1) die("pipe() failed.");

        programs[i].fd_out = readwrite[PIPEFD_WRITE]; // make the output pipe
        programs[i+1].fd_in = readwrite[PIPEFD_READ]; // make the input pipe
    }
    programs[num_programs-1].fd_out = 1; // output end of last program is standard output
}
